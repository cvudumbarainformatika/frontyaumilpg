        ...
        <SearchDropdown v-model="customerSearchModel" placeholder="Cari customer..." :debounce="300"
          ...
          <template #item="{ item }">
            ...
            <div class="text-sm text-secondary-500 dark:text-secondary-400 flex items-center gap-2">
              <span>{{ (item?.category === 'UMUM' ? 'AMBIL / UMUM' : item?.category) || 'Category tidak ada' }}</span>
            </div>
          </template>
        </SearchDropdown>
      </div>
        ...
        <div class="flex justify-between items-start relative">
          <div class="relative w-full">
            ...
            <p class="text-sm text-secondary-500 dark:text-secondary-400 flex items-center mt-1">
              <Icon name="Calculator" class="w-4 h-4 mr-2 text-primary-500" />
              {{ (selectedCustomer?.category === 'UMUM' ? 'AMBIL / UMUM' : selectedCustomer?.category) || 'Category tidak ada' }}
            </p>
            <!-- <p class="text-sm text-secondary-500 dark:text-secondary-400 flex items-center mt-1">
